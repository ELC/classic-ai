...
Por ejemplo, en el hecho le_gusta_a (eduardo,pc) hemos puesto el "gusta" como el
primero de los objetos entre paréntesis y el objeto "es gustado" en segundo
lugar. As\[ el hecho le_gusta_a (eduardo,pc) no es lo mismo que le_gusta_a
...
un término *coma* le_gusta_a (eduardo,pc) podríamos representar lo mismo
simplemente con a(b,c) y recordar que a significa *le_gusta_a,* b significa
...
![Ejemplos de reglas PROLOG: Marco y pájaro](images/ejemplos-reglas-prolog-marco-pajaro.png)
...
As\[ podemos permitir que una variable represente un objeto diferente en cada
uso diferente de la regla. Por ejemplo:
...
predicado dado, como *le_gusta_a.* Podemos darle tanto hechos como reglas, en
...
1. Se intenta satisfacer el segundo.

le_gusta_a(jose,vino).

le gusta_a(jose,maria).

1. El segundo objetivo no se cumple, fracasa.

1. Se realiza la reevaluación, se olvida el valor previo.
...
1. Se cumple por lo tanto el objetivo y PROLOG espera
...
La segunda, padres(X,M,P), devolverá las padres de tatiana si la/s
conjuncionales anteriores fueron verdaderas, en este caso sera M=verónica *y*
P=jorge.
...
- Otra forma de organizar la información es ***situar cerca del comienzo de la
  ...
  parte del "árbol''.*** Para bases de conocimientos grandes, de nuevo puede ser
  difícil determinar
...
1. Por una parte, el ***énfasis en la representación, adquisición y uso def
   conocimiento especializado:*** lo que diferencia específicamente a una
   persona experta en un área del saber de otra que no lo sea radica más sus
...
1. Por otra, el ***reconocimiento de que los sistemas, además de resolver
...
La **Base de hechos** (como se suele denominar a lo que en la figura aparece
como Base de Datos), que contiene el conocimiento declarativo, a nivel de datos,
...
1. Ademas, a diferencia de los humanos, los expertos informatizados nunca
...
1. Otra ventaja de un Sistema experto sobre los expertos humanos es que el
   experto
...
1. Una ventaja final de un sistema experto es que después de tener un experto
