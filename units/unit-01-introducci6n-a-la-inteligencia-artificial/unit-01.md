...
2. **Aproximaciones a la IA 7**

1. Basadas en procesamiento de símbolos 7

2. Aproximaciones subsimbólicas 8

3. **Breve historia de la IA 8**

4. **Mundo especial para agentes con IA. 11**

5. **Sistemas que resuelven problemas de la IA. 13**

1. Acciones que debe llevar a cabo el sistema 19

2. Definición mediante búsqueda en espacio de estados 19

3. Sistemas de producción 22

4. Análisis del problema. 23

5. Características de los Sistemas de producción 30
...
llamar el *Test de Turing* (Turing, 1950). Describiremos este test un poco más
adelante, en esta misma sección, pero primero es importante destacar lo que ya
observara Turing: que la respuesta a la pregunta ¿Pueden pensar las máquinas?
depende de cómo definamos las palabras *máquinas* y *pensar.* Turing podría
haber añadido que la respuesta depende también de cómo se defina la palabra
*pueden.*
...
*hipótesis de/ sistema físico de símbolos* de Newell y Simon (Newell /Simon,
1976) está en oposición directa a las creencias de Searle (y a la noción de
encarnación mencionada anteriormente). Esta hipótesis establece. que *un
sistema físico de símbolos dispone de los medios necesarios y suficientes para
desarrollar una actividad general inteligente.* De acuerdo con Newell y Simon,
un Sistema físico de símbolos es una máquina, tal como un ordenador digital,
que es capaz de manipular datos simbólicos - sumar números,• reordenar listas de
símbolos (por ejemplo, ordenar alfabéticamente una lista de nombres), reemplazar
algunos símbolos por otros, etc. Un aspecto importante de esta hipótesis es que
no importa de que este hecho el sistema físico de símbolos. Decimos que la
hipótesis de Newell y Simon es I "neutral respecto al sustrato".
...
Citemos del articulo de Turing (Turing, 1950):
...
Ahora podemos plantearnos la siguiente cuestión: "¿Qué sucedería *si una
máquina interpretase el papel de A en el juego?"* ¿El interrogador hará tantas
...
poco avezado, aunque es un diálogo completamente vano (Weizenbaum, 1965). El
...
"sistema consejero" (McCarthy, 1958), este estilo de IA representa el
...
Studies (Shannon y McCarthy, 1956). La mayoría de los artículos del volumen
...
*"sistema consejero"* (McCarthy, 1958). A este sistema, en lugar de programarlo,
había que decirle lo que necesitaba saber para resolver un problema. Una
modesta, pero influyente, implementación de estas ideas fue abordada por Cordell
Green en su sistema QA3 (Green, 1969). Como resultado de muchas controversias
...
anteriormente (Turing, 1950). Durante el mismo periodo, Warren McCulloch y
Walter Pitts teorizaban sobre las *relaciones entre elementos computacionales
...
la sinergia de sofisticados algoritmos de búsqueda, ordenadores de alta
...
| 9 | (x,y) si x+y | <= | 4 e y > 0 | * (x+y,0) | Verter todo el agua de la jarra de 3 litros en la jarra de 4 litres |

| 10 | (x,y) Si x+y | <= | 3 y X > 0 | * (0,x+y) | Verter todo el agua de la jarra de 4 litros |
...
| Litres en la Jarra de 4 litros | Litres en la jarra de 3 litros | Regla aplicada |
...
de consecuencia-cierta. Asf, uno de los tipos de problemas más difíciles de
resolver son los irrecuperables de consecuencia-incierta. Ejemplos de tales
...
La. palabra "plato" es el objeto directo del verbo "comer". Es posible comerse
un plato, pero es más usual comerse la ensalada de pasta que el plato..

Una ensalada de pasta es una ensalada que contiene pasta. Sin embargo, existen
otras formas semánticas de agrupar dos nombres. Por ejemplo, la comida de perros
normalmente no contiene perros.
...
completa de la misma. Sin embargo, para resolver el problema de encontrar esta
interpretación se necesita generar solo la interpretación misma. No es necesario
ningún registro del proceso seguido para encontrar la interpretación.

Compare esto con el problema de las jarras de agua. En este caso no basta con
indicar que se ha resuelto el problema y que la solución es (2,0). Para este
tipo de problemas, lo que realmente se pide no es el estado final, sino el
camino que se ha seguido para encontrar ese estado. Asf, la solución a este
...
El hecho de que si la mayor exigencia que se tiene es que mejore la educación,
probablemente se esta apoyando a los demócratas.

El hecho de que si uno se opone a un gran aparato del Estado, probablemente se
esta apoyando a los Republicanos.
